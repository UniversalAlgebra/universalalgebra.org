...
interval $[H, G]$ in its subgroup lattice $\mathrm{Sub}(G)$, then $G$ must possess property $P$.
...
assigned one of these five types, transforming $\mathrm{Con}(A)$ into a **labeled congruence lattice**.2 This
...
I[γi, γi+1] is abelian.[[12](#works-cited)]
...
+  **Perspectivity and Projectivity**. Two prime quotients I1​=I[α, β] and I2​=I[γ, δ]
   in $\mathrm{Con}(A)$ are **perspective** if they are opposite sides of a diamond (M3
...
+  **M3 (Diamond) Subconfigurations**. If L≅$\mathrm{Con}(A_G)$ contains an M3 sublattice, then
   ...
   $\mathrm{Con}(A)$ for any A∈V; SD(∧) is equivalent to omitting types {1,2}.8 Thus, the
